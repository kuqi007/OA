...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a
			href="${ctx}/oa/oaNotify/${oaNotify.self?'self':''}">我的公告</a></li>
		<c:if test="${!oaNotify.self}">
			<shiro:hasPermission name="oa:oaNotify:edit">
				<li><a href="${ctx}/oa/oaNotify/form">公告添加</a></li>
			</shiro:hasPermission>
		</c:if>
	</ul>
	...
	<table id="contentTable"
		...
		<tbody>
			<c:forEach items="${page.list}" var="oaNotify">
				<tr>
					<td><a
						href="${ctx}/oa/oaNotify/view?id=${oaNotify.id}">
					...
					<c:if test="${!requestScope.oaNotify.self}">
						...
						<%-- <c:if test="${oaNotify.status eq '1'}"> --%>
							<td><a href="${ctx}/oa/oaNotify/form?id=${oaNotify.id}" onclick=" if(${oaNotify.status}=='1'){return alertx('已发布的公告不能修改', this.href)}">修改</a><%-- </c:if> --%>
								<a href="${ctx}/oa/oaNotify/delete?id=${oaNotify.id}"
								onclick="return confirmx('确认要删除该公告吗？', this.href)">删除</a></td>
						</shiro:hasPermission>
					</c:if>
				</tr>
			</c:forEach>
