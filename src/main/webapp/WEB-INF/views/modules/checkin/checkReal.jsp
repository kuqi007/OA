...
<body>
	...
	<form:form id="searchForm"
		action="${ctx}/checkreal/listOaCheckrealByPage" method="post"
		...
		<div>
			...
			<label>姓名：</label><input id="condition.user.name"
				name="condition.user.name" type="text" maxlength="50"
				class="input-mini" value="${condition.user.name}" /> <label>工号：</label><input
				id="condition.user.no" name="condition.user.no" type="text"
				maxlength="50" class="input-mini" value="${condition.user.no}" /> <label>部门：</label>
			<sys:treeselect id="office" name="condition.office.id"
				value="${condition.office.id}" labelName="condition.office.name"
				labelValue="${condition.office.name}" title="部门"
				url="/sys/office/treeData?type=2&isAll=true" cssClass="input-small"
