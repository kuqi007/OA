	...
	<title>换休单列表</title>
		...
		$(function(){
			$("#btnSubmit").click(function(){
				/* var state=${state};
				if(state==0){
					$("#searchForm").attr("action","${ctx}/work/shopping/listflowType");
				}else{
					$("#searchForm").attr("action","${ctx}/work/shopping/list");
				} */
...
<ul class="nav nav-tabs">
		<li class="active">
		<c:if test="${state==0 }"><a href="${ctx}/work/shopping/listflowType">采购申请单列表</a></c:if>
		<c:if test="${state!=0 }"><a href="${ctx}/work/shopping/list">采购申请单列表</a></c:if>
		</li>
		<li><c:if test="${state==0 }"><a href="${ctx}/work/shopping/edit">添加采购申请单</a></c:if></li>
</ul><br/>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<tbody>
			...
			<tr>
					...
					<td>
	    				<a href="${ctx}/work/shopping/edit?item.id=${item.id}&state=${state}">查看</a>
						<shiro:hasPermission name="work:shopping:delete"><a href="${ctx}/work/shopping/del?item.id=${item.id}" onclick="return confirmx('确认要删除该条记录吗？', this.href)">删除</a></shiro:hasPermission>
						<c:if test="${state==0 }"><a href="${ctx}/work/shopping/termination?item.id=${item.id}" onclick="return confirmx('确认要终止该条记录吗？', this.href)">终止</a></c:if>
					</td>
					<td>
					...
					<td>
						<c:if test="${item.storage==0 }">未入库</c:if>
						<c:if test="${item.storage==1 }">已入库</c:if>
					</td>
					<%-- <td><c:if test="${item.flagPayment==0 }">未付款</c:if>
					 	<c:if test="${item.flagPayment==1 }">已付款</c:if> 
					</td> --%>
				</tr>
	</c:forEach>
		</tbody>
