...
/* 		$("#btnSubmit").click(function(){
			if(state==0){
				$("#searchForm").attr("action","${ctx}/work/leave/listflowType");
			}else{
				$("#searchForm").attr("action","${ctx}/work/leave/list");
			}
		}); */
...
<body>
	<ul class="nav nav-tabs">
		...
		<li><c:if test="${state==0 }"><a href="${ctx}/work/leave/edit">添加请假单</a></c:if></li>
	</ul>
	<br />
		...
		<tbody>
			<c:forEach var="item" items="${page.list }">
				<tr>
					...
					<td><a href="${ctx}/work/leave/edit?item.id=${item.id}&state=${state}">查看</a>
						<shiro:hasPermission name="work:leave:delete"><a href="${ctx}/work/leave/del?item.id=${item.id}" onclick="return confirmx('确认要删除该条记录吗？', this.href)">删除</a> </shiro:hasPermission>
						<c:if test="${state==0 }"><a href="${ctx}/work/leave/termination?item.id=${item.id}" onclick="return confirmx('确认要终止该条记录吗？', this.href)">终止</a></c:if>
					</td>
					<td>
