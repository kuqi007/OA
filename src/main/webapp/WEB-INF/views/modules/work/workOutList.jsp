	...
	<title>外出单列表</title>
		...
		$(function(){
			$("#btnSubmit").click(function(){
				/* var state=${state};
				if(state==0){
					$("#searchForm").attr("action","${ctx}/work/out/listflowType");
				}else{
					$("#searchForm").attr("action","${ctx}/work/out/list");
				} */
			});
...
<ul class="nav nav-tabs">
		<li class="active">
		<c:if test="${state==0 }"><a href="${ctx}/work/out/listflowType">外出单列表</a></c:if>
		<c:if test="${state!=0 }"><a href="${ctx}/work/out/list">外出单列表</a></c:if>
		</li>
		<li><c:if test="${state==0 }"><a href="${ctx}/work/out/edit">添加外出单</a></c:if></li>
</ul><br/>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<tbody>
			...
			<tr>
					...
					<td>
	    				<a href="${ctx}/work/out/edit?item.id=${item.id}&state=${state}">查看</a>
						<shiro:hasPermission name="work:out:delete"><a href="${ctx}/work/out/del?item.id=${item.id}" onclick="return confirmx('确认要删除该条记录吗？', this.href)">删除</a></shiro:hasPermission>
						<c:if test="${state==0 }"><a href="${ctx}/work/out/termination?item.id=${item.id}"onclick="return confirmx('确认要终止该条记录吗？', this.href)">终止</a></c:if>
					</td>
					<td>
