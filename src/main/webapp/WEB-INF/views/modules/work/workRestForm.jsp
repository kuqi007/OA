...
<body>
	<ul class="nav nav-tabs">
		<li>
		<c:if test="${state==0 }"><a href="${ctx}/work/rest/listflowType${flowType}">换休单列表</a></c:if>
		<c:if test="${state!=0 }"><a href="${ctx}/work/rest/list${state}?flowType=${flowType}">换休单列表</a></c:if>
		</li>
		<li class="active"><a href="${ctx}/work/rest/edit?item.id=${item.id}&state=${state}&flowType=${flowType}">换休单${not empty item.id?'查看':'添加'}</a></li>
	</ul><br/>
	...
	<form:form id="inputForm" modelAttribute="workRestForm" action="${ctx}/work/rest/save?state=${state }" method="post" class="form-horizontal">
		...
		<div class="control-group">
			<label class="control-label">换休原由:</label>
			<div class="controls">
		...
		<div class="control-group">
			<label class="control-label">换休时间:</label>
			<div class="controls">
				...
				总共换休时长:<input id="dayTime" class="input required digits" style="width:50px;" type="text" maxlength="20"name="item.dayTime">（小时）
				<span class="help-inline"><font color="red">*</font> </span>
			</div>
		</div>
		<div class="control-group">
		...
		<div class="control-group">
			...
			<div class="controls">
			<label class="lbl">${item.departmentName }</label>
			<input type="hidden" name="item.department" value="${item.department }"/>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">职务:</label>
		...
		<div class="control-group">
			...
			<label class="lbl"><fmt:formatDate value='${item.timeBegin }' pattern='yyyy-MM-dd HH:mm'/> -- <fmt:formatDate value='${item.timeEnd }' pattern='yyyy-MM-dd HH:mm'/>总共换休时长:${item.dayTime }（小时）</label>
