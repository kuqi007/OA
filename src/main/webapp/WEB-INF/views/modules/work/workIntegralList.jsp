...
<script type="text/javascript">
	...
	$(function(){
		$("#btnSubmit").click(function(){
			/* var state=${state};
			if(state==0){
				$("#searchForm").attr("action","${ctx}/work/integral/listflowType");
			}else{
				$("#searchForm").attr("action","${ctx}/work/integral/list");
			} */
...
<body>
	<ul class="nav nav-tabs">
		...
		<c:if test="${state!=0 }"><a href="${ctx}/work/integral/list${state}">积分撤销申请单列表</a></c:if>
		</li>
		<li><c:if test="${state==0 }"><a href="${ctx}/work/integral/edit">添加积分撤销申请单</a></c:if></li>
	</ul>
	<br />
		...
		<tbody>
			<c:forEach var="item" items="${page.list }">
				<tr>
					...
					<td>
						<c:if test="${item.frozen==0 }">未冻结</c:if>
						<c:if test="${item.frozen==1 }">已冻结</c:if>
					</td>
					<td>
						<c:if test="${item.record==0 }">未入档</c:if>
						<c:if test="${item.record==1 }">已入档</c:if>
					</td>
					<td><a href="${ctx}/work/integral/edit?item.id=${item.id}&state=${state }">查看</a>
						<shiro:hasPermission name="work:integral:delete"><a href="${ctx}/work/integral/del?item.id=${item.id}" onclick="return confirmx('确认要删除该条记录吗？', this.href)">删除</a> </shiro:hasPermission>
						<c:if test="${state==0 }"><a href="${ctx}/work/integral/termination?item.id=${item.id}" onclick="return confirmx('确认要终止该条记录吗？', this.href)">终止</a></c:if>
					</td>
					<td>
