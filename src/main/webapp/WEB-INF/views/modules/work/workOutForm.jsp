...
<body>
	<ul class="nav nav-tabs">
		<li>
		<c:if test="${state==0 }"><a href="${ctx}/work/out/listflowType${flowType}">外出单列表</a></c:if>
		<c:if test="${state!=0 }"><a href="${ctx}/work/out/list${state}?flowType=${flowType}">外出单列表</a></c:if>
		</li>
		<li class="active"><a href="${ctx}/work/out/edit?item.id=${item.id}&state=${state}&flowType=${flowType}">外出单${not empty item.id?'查看':'添加'}</a></li>
	</ul><br/>
	...
	<form:form id="inputForm" modelAttribute="workOutForm" action="${ctx}/work/out/save?state=${state }" method="post" class="form-horizontal">
		...
		<div class="control-group">
			<label class="control-label">外出原由:</label>
			<div class="controls">
			<textarea rows="2" id="reason" name="item.reason"></textarea>
		...
		<div class="control-group">
			...
			<div class="controls">
				<input id="beginDate" class="input Wdate required" style="width:160px;" type="text" readonly="readonly" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm',isShowClear:false});"  maxlength="20" name="item.timeBegin">
				<span class="help-inline"><font color="red">*</font> </span>
				<label> -- </label>
				<input id="endDate" class="input Wdate required" style="width:160px;" type="text" readonly="readonly" maxlength="20" name="item.timeEnd">
				<span class="help-inline"><font color="red">*</font> </span>
				总共外出时长:<input id="dayTime" class="input required digits" style="width:50px;" type="text" maxlength="20"name="item.dayTime">（小时）
				<span class="help-inline"><font color="red">*</font> </span>
			</div>
		</div>
		<div class="control-group">
		...
		<div class="control-group">
			<label class="control-label">外出时间:</label>
			<div class="controls">
			<label class="lbl"><fmt:formatDate value='${item.timeBegin }' pattern='yyyy-MM-dd HH:mm'/> -- <fmt:formatDate value='${item.timeEnd }' pattern='yyyy-MM-dd HH:mm'/>总共外出时长：<label>${item.dayTime }</label>（小时）</label>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">单据紧急情况:</label>
