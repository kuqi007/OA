...
<body>
	<ul class="nav nav-tabs">
		<li>
		<c:if test="${state==0 }"><a href="${ctx}/work/travel/listflowType">出差申请单列表</a></c:if>
		<c:if test="${state!=0 }"><a href="${ctx}/work/travel/list">出差申请单列表</a></c:if>
		</li>
		<li class="active"><a href="${ctx}/work/travel/edit?item.id=${item.id}">出差申请单${not empty item.id?'查看':'添加'}</a></li>
	</ul><br/>
	...
	<form:form id="inputForm" modelAttribute="workTravelForm" action="${ctx}/work/travel/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			<label class="control-label">出差原由:</label>
			<div class="controls">
		...
		<div class="control-group">
			...
			<div class="controls">
				<input id="beginDate" class="input Wdate required" style="width:160px;" type="text" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm',isShowClear:false});" value="<fmt:formatDate value='${item.timeBegin }' pattern='yyyy-MM-dd HH:mm'/>" maxlength="20" readonly="readonly" name="item.timeBegin">
				<span class="help-inline"><font color="red">*</font> </span>
				<label> -- </label>
				<input id="endDate" class="input Wdate required" style="width:160px;" type="text" value="<fmt:formatDate value='${item.timeEnd }' pattern='yyyy-MM-dd HH:mm'/>" maxlength="20" readonly="readonly" name="item.timeEnd">
				<span class="help-inline"><font color="red">*</font> </span>
				总共出差时长:<input id="dayTime" class="input required digits" style="width:50px;" type="text" maxlength="20"name="item.dayTime">（天）
				<span class="help-inline"><font color="red">*</font> </span>
			</div>
		</div>
		<div class="control-group">
		...
		<div class="control-group">
			...
			<div class="controls">
                <sys:treeselect id="vehicle" name="item.vehicle" value="${item.vehicle}" labelName="item.vehicleName" labelValue="${item.vehicleName}"
					title="交通工具" url="/work/travel/vehicleList?type=work_vehicle" cssClass="required" notAllowSelectParent="true"/>
					<span class="help-inline"><font color="red">*</font> </span>
			</div>
		</div>
		<div class="control-group">
		...
		<div class="control-group">
			<label class="control-label">出差时间:</label>
			<div class="controls">
			<label class="lbl"><fmt:formatDate value='${item.timeBegin }' pattern='yyyy-MM-dd HH:mm'/>
			 -- <fmt:formatDate value='${item.timeEnd }' pattern='yyyy-MM-dd HH:mm'/>
			总共出差时长:${item.dayTime }（天）</label>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">出差地点:</label>
		...
		<div class="control-group">
			<label class="control-label">交通工具:</label>
			<div class="controls">
			<label class="lbl">${item.vehicleName }</label>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">单据紧急情况:</label>
