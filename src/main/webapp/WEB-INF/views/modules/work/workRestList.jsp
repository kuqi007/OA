...
/* 				var state=${state};
				if(state==0){
					$("#searchForm").attr("action","${ctx}/work/rest/listflowType");
				}else{
					$("#searchForm").attr("action","${ctx}/work/rest/list");
				}
				 */
			});
...
<ul class="nav nav-tabs">
		<li class="active">
		<c:if test="${state==0 }"><a href="${ctx}/work/rest/listflowType">换休单列表</a></c:if>
		<c:if test="${state!=0 }"><a href="${ctx}/work/rest/list">换休申请单列表</a></c:if>
		</li>
		<li><c:if test="${state==0 }"><a href="${ctx}/work/rest/edit">添加换休申请单</a></c:if></li>
</ul><br/>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<tbody>
			...
			<tr>
					...
					<td>
	    				<a href="${ctx}/work/rest/edit?item.id=${item.id}&state=${state}">查看</a>
						<shiro:hasPermission name="work:rest:delete"><a href="${ctx}/work/rest/del?item.id=${item.id}" onclick="return confirmx('确认要删除该条记录吗？', this.href)">删除</a></shiro:hasPermission>
						<c:if test="${state==0 }"><a href="${ctx}/work/rest/termination?item.id=${item.id}"onclick="return confirmx('确认要终止该条记录吗？', this.href)">终止</a></c:if>
					</td>
					<td>
