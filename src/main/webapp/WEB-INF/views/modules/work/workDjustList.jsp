...
<script type="text/javascript">
	...
	$(function(){
		$("#btnSubmit").click(function(){
			/* var state=${state};
			if(state==0){
				$("#searchForm").attr("action","${ctx}/work/djust/listflowType");
			}else{
				$("#searchForm").attr("action","${ctx}/work/djust/list");
			} */
...
<body>
	<ul class="nav nav-tabs">
		<li class="active">
		<c:if test="${state==0 }"><a href="${ctx}/work/djust/listflowType">价格调整审批单列表</a></c:if>
		<c:if test="${state!=0 }"><a href="${ctx}/work/djust/list">价格调整审批单列表</a></c:if>
		</li>
		<li><c:if test="${state==0 }"><a href="${ctx}/work/djust/edit">添加价格调整审批单</a></c:if></li>
	</ul>
	<br />
		...
		<tbody>
			<c:forEach var="item" items="${page.list }">
				<tr>
					...
					<td><a href="${ctx}/work/djust/edit?item.id=${item.id}&state=${state}">查看</a>
						<shiro:hasPermission name="work:djust:delete"><a href="${ctx}/work/djust/del?item.id=${item.id}" onclick="return confirmx('确认要删除该条记录吗？', this.href)">删除</a> </shiro:hasPermission>
						<c:if test="${state==0 }"><a href="${ctx}/work/djust/termination?item.id=${item.id}" onclick="return confirmx('确认要终止该条记录吗？', this.href)">终止</a></c:if>
						</td>
					<td>
