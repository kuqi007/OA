	...
	<title>招聘需求单列表</title>
		...
		$(function(){
			$("#btnSubmit").click(function(){
				/* var state=${state};
				if(state==0){
					$("#searchForm").attr("action","${ctx}/work/recruit/listflowType");
				}else{
					$("#searchForm").attr("action","${ctx}/work/recruit/list");
				} */
...
<ul class="nav nav-tabs">
		...
		<li><c:if test="${state==0 }"><a href="${ctx}/work/recruit/edit">添加招聘申请单</a></c:if></li>
</ul><br/>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<tbody>
			...
			<tr>
					...
					<td>
	    				<a href="${ctx}/work/recruit/edit?item.id=${item.id}&state=${state}">查看</a>
						<shiro:hasPermission name="work:recruit:delete"><a href="${ctx}/work/recruit/del?item.id=${item.id}" onclick="return confirmx('确认要删除该条记录吗？', this.href)">删除</a></shiro:hasPermission>
						<c:if test="${state==0 }"><a href="${ctx}/work/recruit/termination?item.id=${item.id}"onclick="return confirmx('确认要终止该条记录吗？', this.href)">终止</a></c:if>
					</td>
					<td>
