...
/* 		function div(){
			...
			var leave=date%(24*3600*1000);//计算天数后剩余的毫秒数  
			var hours=Math.floor(leave/(3600*1000)); // 小时
			$("#timeLabel").text(days+"天"+hours+"小时");
		} */
		function exportBtna(){
			$("#expor").submit();
		}
...
<body>
	<ul class="nav nav-tabs">
		<li>
		<c:if test="${state==0 }"><a href="${ctx}/work/leave/listflowType">请假单列表</a></c:if>
		<c:if test="${state!=0 }"><a href="${ctx}/work/leave/list">请假单列表</a></c:if>
		</li>
		<li class="active"><a href="${ctx}/work/leave/edit?item.id=${item.id}">请假单${not empty item.id?'查看':'添加'}</a></li>
	</ul><br/>
	...
	<form:form id="inputForm" modelAttribute="workLeaveForm" action="${ctx}/work/leave/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
			<label class="lbl">
				<input id="type" type="hidden" value="${item.type }"/>
				<input class="typere" name="item.type" value="3" type="radio" checked="checked"/>公假
				<input class="typere" name="item.type" value="4" type="radio"/>病假
				<input class="typere" name="item.type" value="5" type="radio"/>事假
				<input class="typere" name="item.type" value="6" type="radio"/>年假
				<input class="typere" name="item.type" value="7" type="radio"/>其他
				</label>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">请假原由:</label>
			<div class="controls">
				<textarea id="reason" rows="2" name="item.reason" class="required"></textarea>
			</div>
		</div>
		<div class="control-group">
			...
			<div class="controls">
				...
				<span class="help-inline"><font color="red">*</font> </span>
				总共请假时长:<input id="dayTime" class="input required digits" style="width:50px;" type="text" maxlength="20"name="item.dayTime">（小时）
				<span class="help-inline"><font color="red">*</font> </span>
				<!-- <label id="sumDate"><u>单击显示</u></label> -->
			</div>
		</div>
		<div class="control-group">
		...
		<div class="control-group">
			...
			<div class="controls">
			<label class="lbl">
			<c:if test="${item.type==3 }">公假</c:if>
			<c:if test="${item.type==4 }">病假</c:if>
			<c:if test="${item.type==5 }">事假</c:if>
			<c:if test="${item.type==6 }">年假</c:if>
			<c:if test="${item.type==7 }">其他</c:if>
			<input id="type" type="hidden" name="item.type" value="${item.type }"/>
			</label>
			</div>
		</div>
		<div class="control-group">
	...
	<!-- 专为导出而用，吼吼吼^O^  -->
	<form action="${ctx}/work/leave/exportLeave" method="post" id="expor">
		<input id="id" value="${item.id }" type="hidden" name="item.id"/>
	</form>
</body>
</html>