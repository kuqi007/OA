...
<script type="text/javascript">
	...
	$(function(){
		$("#btnSubmit").click(function(){
			/* var state=${state};
			if(state==0){
				$("#searchForm").attr("action","${ctx}/work/reimbursement/listflowType");
			}else{
				$("#searchForm").attr("action","${ctx}/work/reimbursement/list");
			} */
...
<body>
	<ul class="nav nav-tabs">
		...
		<li><c:if test="${state==0 }"><a href="${ctx}/work/reimbursement/edit">添加差旅报销单</a></c:if></li>
	</ul>
	<br />
		...
		<tbody>
			<c:forEach var="item" items="${page.list }">
				<tr>
					...
					<td><a href="${ctx}/work/reimbursement/edit?item.id=${item.id}&state=${state }">查看</a>
						<shiro:hasPermission name="work:reimbursement:delete"><a href="${ctx}/work/reimbursement/del?item.id=${item.id}"onclick="return confirmx('确认要删除该条记录吗？', this.href)">删除</a></shiro:hasPermission>
						<c:if test="${state==0 }"><a href="${ctx}/work/reimbursement/termination?item.id=${item.id}"onclick="return confirmx('确认要终止该条记录吗？', this.href)">终止</a></c:if>
					</td>
					<td><c:if test="${item.flag==0 }">审批中</c:if>
