...
<head>
	<title>出差单列表</title>
		...
		$(function(){
			...
			/* 	var state=${state};
				if(state==0){
					$("#searchForm").attr("action","${ctx}/work/travel/listflowType");
				}else{
					$("#searchForm").attr("action","${ctx}/work/travel/list");
				}
				 */
			});
...
<ul class="nav nav-tabs">
		<li class="active">
		<c:if test="${state==0 }"><a href="${ctx}/work/travel/listflowType${flowType}">出差申请单列表</a></c:if>
		<c:if test="${state!=0 }"><a href="${ctx}/work/travel/list${state}?flowType=${flowType}">出差申请单列表</a></c:if>
		</li>
		<li><c:if test="${state==0 }"><a href="${ctx}/work/travel/edit?state=${state}&flowType=${flowType}">添加出差申请单</a></c:if></li>
</ul><br/>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<tbody>
			...
			<tr>
					...
					<td>
	    				<a href="${ctx}/work/travel/edit?item.id=${item.id}&state=${state }">查看</a>
						<shiro:hasPermission name="work:travel:delete"><a href="${ctx}/work/travel/del?item.id=${item.id}" onclick="return confirmx('确认要删除该条记录吗？', this.href)">删除</a></shiro:hasPermission>
						<c:if test="${state==0 }"><a href="${ctx}/work/travel/termination?item.id=${item.id}" onclick="return confirmx('确认要终止该条记录吗？', this.href)">终止</a></c:if>
					</td>
					<td>
