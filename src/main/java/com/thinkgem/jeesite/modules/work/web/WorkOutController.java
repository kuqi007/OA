...
import com.thinkgem.jeesite.modules.work.form.WorkOutForm;
...
@Controller
@Scope("prototype")
@RequestMapping(value = "${adminPath}/work/out")
public class WorkOutController extends BaseController {

	@Resource(name = "workOutServiceImpl")
	...
	@RequiresPermissions("work:out:list:flowType")
	@RequestMapping(value = "/listflowType")
	public String index0(WorkOutForm workOutForm, HttpServletRequest request, HttpServletResponse response, Model model) {
		User user=UserUtils.getUser();// 获取当前用户
		Page<WorkOut> page=workOutForm.getPage();// 获取分页对象
		WorkOutCondition condition=workOutForm.getCondition(); 
		// 根据用户id  查询用户所写单据
		condition.setCreateBy(user.getId());
		condition.setFlag(0);
		workOutService.listWorkOutByPage(page,condition);
		model.addAttribute("page",page);
		model.addAttribute("state", 0);
		model.addAttribute("condition", condition);
		return "modules/work/workOutList";
	}
	
	@RequiresPermissions("work:out:list")
	@RequestMapping(value = "/list")
	public String index1(@ModelAttribute WorkOutForm workOutForm, HttpServletRequest request, HttpServletResponse response, Model model) {
		Page<WorkOut> page=workOutForm.getPage();// 获取分页对象
		WorkOutCondition condition=workOutForm.getCondition();
		User user=UserUtils.getUser();// 获取当前用户
	...
	@RequiresPermissions("work:out:listAllof")
	@RequestMapping(value = "/listAllof")
	public String listAllof(@ModelAttribute WorkOutForm workOutForm, HttpServletRequest request, HttpServletResponse response, Model model) {
		User user=UserUtils.getUser();// 获取当前用户
		Page<WorkOut> page=workOutForm.getPage();// 获取分页对象
		WorkOutCondition condition=workOutForm.getCondition();
	...
	@RequiresPermissions("work:out:listAll")
	@RequestMapping(value = "/listAll")
	public String listAll(@ModelAttribute WorkOutForm workOutForm, HttpServletRequest request, HttpServletResponse response, Model model) {
		Page<WorkOut> page=workOutForm.getPage();// 获取分页对象
		WorkOutCondition condition=workOutForm.getCondition();
		workOutService.listWorkOutByPage(page,condition);
		model.addAttribute("page",page);
	...
	// 已完成工作
	@RequiresPermissions("work:out:list6")
	@RequestMapping(value = "/list6")
	public String list6(@ModelAttribute WorkOutForm workOutForm, HttpServletRequest request, HttpServletResponse response, Model model) {
		Page<WorkOut> page=workOutForm.getPage();// 获取分页对象
		WorkOutCondition condition=workOutForm.getCondition();
		User user=UserUtils.getUser();// 获取当前用户
	...
	public String edit(@ModelAttribute WorkOutForm workOutForm, HttpServletRequest request, HttpServletResponse response, Model model) {
		if(StringUtils.isNotBlank(workOutForm.getItem().getId())){
			WorkOut item = workOutService.getWorkOutById(workOutForm.getItem().getId());
			...
			if (!item.getPeople5().equals("")) {
				...
				signVo.setText(item.getText5());
				signVo.setSign(item.getSign5());
				signVo.setSignName(systemService.getUser(item.getPeople5()).getRoleList().get(0).getName());
				listSign.add(signVo);
			}
			if (office!=null) {
			...
			if(workOutForm.getState()!=null&&workOutForm.getState()==100)model.addAttribute("state", 0);
		...
		return "modules/work/workOutForm";
	}
	
	// 保存
	@RequestMapping(value = "/save")
	public String save(@ModelAttribute WorkOutForm workOutForm, HttpServletRequest request, HttpServletResponse response, Model model,RedirectAttributes redirectAttributes) {
		Integer state=0;
		String url="";
		try {
			WorkOut item = workOutForm.getItem();
			User user=UserUtils.getUser();// 获取当前用户
			...
			if(StringUtils.isBlank(item.getId())){
				...
				workOutService.add(item);
				url="redirect:" + adminPath + "/work/out/listflowType";
			}else{
				workProcess=workProcessService.listWorkProcess(item.getCreateBy(),1);// 创建人流程信息
				state=Integer.valueOf(workOutForm.getItem().getState());// 订单当前步奏
				String id="";
				...
				url="redirect:" + adminPath + "/work/out/list";//根据单据状态，参看对应单据信息	
			}
		} catch (Exception e) {
	...
	public String del(@ModelAttribute WorkOutForm workOutForm,HttpServletRequest request,HttpServletResponse response,Model model){
		try {
			workOutService.delByIds(workOutForm.getItem().getId().split(","));
		} catch (Exception e) {
			String msg = "删除时发生异常：" + e.getMessage();
			logger.error(msg,e);
		}
		return index0(workOutForm,request,response,model);
	}
	/**
	 * 终止
	 */
	@RequestMapping(value = "/termination")
	public String termination(@ModelAttribute WorkOutForm workOutForm,HttpServletRequest request,HttpServletResponse response,Model model){
		workOutService.termination(workOutForm.getItem().getId());
		return index0(workOutForm,request,response,model);
	}
